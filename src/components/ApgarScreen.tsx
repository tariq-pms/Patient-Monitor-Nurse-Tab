    ...
    try {
      ...
      if (response.ok) {
        ...
        if (searchData?.entry && searchData.entry.length > 0) {
          // ⭐ Filter for "Apgar Score" specifically
          const apgarEntry = searchData.entry.find((e: any) =>
            e.resource.code?.text === "Apgar Score" ||
            e.resource.code?.coding?.some((c: any) => c.display === "Apgar score")
          );
          if (apgarEntry) {
            const existingProcedureId = apgarEntry.resource.id;
            setProcedureResourceId(existingProcedureId);
            console.log("Apgar Procedure Resource ID fetched:", existingProcedureId);
          }
        }
      } else {
