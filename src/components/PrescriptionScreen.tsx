...
import { Box, Typography, TextField, Button, Grid, Divider, Paper, Autocomplete, MenuItem, Select, Dialog, DialogTitle, Tooltip, DialogContent, DialogActions, FormControlLabel, FormGroup, Checkbox, FormControl, InputLabel, Snackbar, Alert, CircularProgress, useTheme, createFilterOptions } from "@mui/material";
...
export const PrescriptionScreen: React.FC<PrescriptionScreenProps> = (props) => {
  ...
  const handleDeleteMedication = async (medicationId: string, medicationName: string) => {
    ...
    try {
      const deleteUrl = `${import.meta.env.VITE_FHIRAPI_URL}/MedicationRequest/${medicationId}`;
      ...
      if (response.ok) {
        ...
        setPrescriptionHistory(prev => prev.filter(med => med.id !== medicationId));
