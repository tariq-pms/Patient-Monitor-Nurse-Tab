...
const generatePDF = () => {
  const doc = new jsPDF();
  const pageHeight = doc.internal.pageSize.height; // Get total height (A4 is ~297mm)
// 1. Add Logo (Top Right)
if (logoBase64) {
  doc.addImage(logoBase64, 'PNG', 140, 5, 45, 15); 
}

   // 2. Hospital Header Text
   doc.setFont("helvetica", "bold");
   doc.setFontSize(14);
   doc.text("INPUT OUTPUT CHART", 14, 15);
   
   // 3. Header Fields (Dynamic Patient Info)
  ...
  // Row 1: Patient Name, IPD No, Age/Sex, Birth Date
  doc.text("Patient Name :", 14, 30);
  doc.text("IPD No:", 85, 30);
  doc.text("G.A :", 130, 30);
  doc.text("Sex :", 175, 30);
  // doc.text("DOB :", 175, 35);

  doc.setFont("helvetica", "normal");
  doc.text(`${props.patient_name || '-'}`, 38, 30);
  doc.text(`${props.patient_id || '-'}`, 100, 30);
  doc.text(`${props.gestational_age || '-'}`, 140, 30);
  doc.text("____", 183, 30); // Placeholder for Sex
 

  // Row 2: Ward and Bed
  doc.setFont("helvetica", "bold");
  doc.text("Ward / Room Type :", 14, 37);
  doc.text("Bed No:", 85, 37);
  doc.text("DOB:", 130, 37);
  

  doc.setFont("helvetica", "normal");
  doc.text("___________", 45, 37); // Placeholder if data not available
  doc.text("___________", 100, 37);
  doc.text(`${props.birth_date || '-'}`, 140, 37);
  doc.text(`${formattedDateTime || '-'}`, 170, 37);

  // Row 3: Diagnosis and Operation
  doc.setFont("helvetica", "bold");
  doc.text("Diagnosis :", 14, 44);
  doc.text("Operation :", 130, 44);

  doc.setFont("helvetica", "normal");
  doc.text("__________________________", 32, 44);
  doc.text("__________________________", 148, 44);
  ...
  {chartData.length > 0 ? (
    chartData.map((row, index) => (
      <Box
        ...
        py={1.5}
        sx={{
          borderBottom: index !== chartData.length - 1 ? "1px solid #F3F4F6" : "none",
          "& .MuiTypography-root": { fontSize: "0.75rem", color: "#000" }, // Ensures text is black
        }}
      >
...
<Box display="grid" gridTemplateColumns="repeat(4, 1fr)" gap={1.5} mt={2}>
  ...
  {/* ===== IV Fluid ===== */}
  <Box mt={2}>
    <Typography fontWeight={600} sx={{ color: '#000', mb: 0.5 }}>I.V Fluid</Typography>
    <QuantityInput 
  ...
  {/* ===== By Mouth ===== */}
  <Box mt={2}>
    <Typography fontWeight={600} sx={{ color: '#000', mb: 0.5 }}>By Mouth</Typography>
    <QuantityInput 
  ...
  {/* ===== RT Feed / NG ===== */}
  <Box mt={2}>
    <Typography fontWeight={600} sx={{ color: '#000', mb: 0.5 }}>RT Feed / NG</Typography>
    <QuantityInput 
    ...
    return (
      ...
      >
        <Box display="flex" justifyContent="space-between" alignItems="center">
          <Typography 
            fontSize="0.85rem" 
            sx={{ color: '#000000' }} // Pure black text
        ...
        {isChecked && (
          <TextField
            ...
            InputProps={{
              ...
              endAdornment: (
                <Typography sx={{ color: "#000000", fontSize: "0.75rem", ml: 0.5 }}>
                  ml
                </Typography>
              ),
              style: { 
  ...
  {/* ===== Remarks ===== */}
  <Box mt={2}>
    <Typography fontWeight={600} mb={0.5} sx={{ color: '#000' }} fontSize="0.9rem">
      Remarks
    </Typography>
    <TextField
      fullWidth
      ...
      sx={{
        ...
        "& .MuiOutlinedInput-root": { 
          borderRadius: "10px",
          color: '#000' // Text inside remarks field
        },
      }}
    />
  </Box>
  ...
  <Typography variant="caption" sx={{ color: '#000', opacity: 0.7 }}>
   BY : {props.UserRole}
  </Typography>
