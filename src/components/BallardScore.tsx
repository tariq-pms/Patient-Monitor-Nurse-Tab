    ...
    try {
      ...
      if (response.ok) {
        ...
        if (searchData?.entry && searchData.entry.length > 0) {
          // ⭐ Filter for "Ballard Score Assessment" specifically
          const ballardEntry = searchData.entry.find((e: any) =>
            e.resource.code?.text === "Ballard Score Assessment" ||
            e.resource.code?.coding?.some((c: any) => c.display === "Ballard Score Assessment")
          );
          if (ballardEntry) {
            const existingProcedureId = ballardEntry.resource.id;
            setProcedureResourceId(existingProcedureId);
            console.log("Ballard Procedure Resource ID fetched:", existingProcedureId);
          }
        }
      } else {
