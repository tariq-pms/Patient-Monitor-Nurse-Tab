...
// Top-level component (must be defined outside render fn to avoid React error #130)
const Tick = ({ checked = false }: { checked?: boolean }) => (
  <Checkbox
    checked={checked}
    disabled
    size="small"
...
function readComponent(obs: any, label: string): string {
  ...
  if (c.valueDateTime) return c.valueDateTime;
...
allergies: raw.allergies?.map((a: any) => ({
  ...
  const [vitals, setVitals] = useState({
    ...
    // const now = new Date();
  //   const currentDate = now.toLocaleDateString("en-US"); // DD/MM/YYYY
  ...
  // (Tick is defined at module level above)
...
for (const item of allergyMappings) {
  ...
  if (!response.ok) {
    ...
    console.error("❌ FHIR Server Error Details:", errorData.issue);
      ...
      try {
        const raw = await fetchNurseInitialAssessment(patientId, encounterId);
        const hasData = raw.patientDetails || raw.vitals || raw.anthropometry || raw.medications?.length > 0 || raw.allergies?.length > 0 || raw.carePlan;
        if (hasData) {
          setAssessment(buildNurseReport(raw));
        } else {
          setAssessment(null);
        }
      } catch (e) {
        console.error("Failed to reload assessment data", e);
      }
...
const handleCloseSnackbar = () => setSnackbar({ ...snackbar, open: false });
  ...
  // Sync props to local state on load
useEffect(() => {
  if (props.patient) {
    setPatient({
      name: props.patient_name || "",
      gender: genderValue || "",
      doa: getDatePart(props.patient.admissionDate) || "",
      admittedTime: getTimePart(props.patient.admissionDate) || "",
      // admissionDate: getDatePart(props.patient.admissionDate) || "",
      age: props.gestational_age || "",
      uhid: props.patientId1 || "",
      admissionNo: props.admissionNo || "",
      companionName: props.patient.kinName || "",
      relationship: props.patient.kinRelation || "",
      language: "English" // Default value
    });
    console.log('checking props working fine companionName',props.patient.kinName);
    console.log('checking props working fine relationship',props.patient.kinRelation);
    
    // If you have a separate state for relation
...
const downloadPDF = async () => {
  ...
  <IconButton 
    disabled={!assessment || Object.keys(assessment.vitals).length === 0} // Disables if no data
    // onClick={downloadPDF}
    sx={{
...
{loading ? (
  ...
  <Box sx={{ p: 4, textAlign: 'center' }}>
    <CircularProgress size={24} sx={{ mb: 2, color: "#228BE6" }} />
    <Typography variant="body2" color="textSecondary">
      Fetching Nurse Assessment...
    </Typography>
  </Box>
) : !assessment || Object.keys(assessment?.vitals || {}).length === 0 ? (
  /* --- 2. NO REPORT FOUND STATE --- */
  <Box sx={{ 
    p: 6, textAlign: 'center', border: '2px dashed #DEE2E6', 
    borderRadius: '16px', bgcolor: '#F8F9FA', mt: 2 
  }}>
    <Typography variant="h6" color="textSecondary" gutterBottom>
      No Nurse Assessment Found
    </Typography>
    <Typography variant="body2" color="textSecondary">
      There is no nurse initial assessment data recorded for this Patient.
    </Typography>
    <Button 
      variant="outlined" 
      startIcon={<AddIcon />} 
  ...
  /* --- 3. REPORT FOUND STATE (Summary Card) --- */
  <Card 
  ...
  >
    <Box>
      <Typography variant="subtitle2" fontWeight={700} color="#1A1B1E">
        Nurse Initial Assessment Report
      </Typography>
      <Typography variant="caption" color="textSecondary">
        Patient: {props?.patient_name || "N/A"} | UHID: {props?.patientId1 || "N/A"}
      </Typography>
    </Box>
    <Chip 
        label="View Report" 
...
>
  ...
  <Box sx={{ border: '1px solid #d1d9e0', borderRadius: '8px', p: 1.5, bgcolor: '#fff' }}>
    <Grid container justifyContent="space-between" alignItems="center" sx={{ mb: 0.5 }}>
      <Grid item display="flex" gap={1}>
        ...
        <Typography  variant="subtitle2" sx={{ fontWeight: 'bold'}}>{patient.name || props.patient_name}</Typography>
      </Grid>
      <Grid item display="flex" gap={1}>
        <Typography variant="subtitle2" sx={{ color: '#90a4ae'}}>UHID:</Typography>
        <Typography variant="subtitle2" sx={{ fontWeight: 'bold'}}>{patient.uhid || props.patientId1}</Typography>
      </Grid>
    </Grid>
    ...
    <Grid container justifyContent="space-between" alignItems="center">
      <Grid item display="flex" gap={1}>
        ...
        <Typography variant="subtitle2" sx={{ fontWeight: 'bold'}}>{patient.age || props.gestational_age}</Typography>
      </Grid>
      <Grid item display="flex" gap={1}>
        <Typography variant="subtitle2" sx={{ color: '#90a4ae'}}>ADMISSION NO:</Typography>
        <Typography variant="subtitle2" sx={{ fontWeight: 'bold' }}>{patient.admissionNo || "-"}</Typography>
      </Grid>
    </Grid>
  </Box>
</Box>
    ...
    <Stack sx={{p:2}}>
      ...
      <Box sx={{ border: '1px solid #f0f0f0', borderRadius: '4px', p: 1, mb: 3 }}>
        <Grid container spacing={1} sx={{ fontSize: '10px' }}>
          <Grid item xs={2} color="#666">Patient Name</Grid>
          <Grid item xs={4}><strong>{patient.name}</strong></Grid>
          <Grid item xs={2} color="#666">UHID / IP No.</Grid>
          <Grid item xs={4}><strong>{patient.uhid}</strong></Grid>
          
          <Grid item xs={2} color="#666">Sex</Grid>
          <Grid item xs={4}><strong>{patient.gender}</strong></Grid>
          <Grid item xs={2} color="#666">Gestation Age</Grid>
          <Grid item xs={4}><strong>{patient.age}</strong></Grid>
          
          <Grid item xs={2} color="#666">Date of Admission</Grid>
          <Grid item xs={4}><strong>{patient.doa}</strong></Grid>
          <Grid item xs={2} color="#666">Time of Admission</Grid>
          <Grid item xs={4}><strong>{patient.admittedTime}</strong></Grid>
        </Grid>
      </Box>

      {/* 02. COMPANION & LANGUAGE */}
      <Typography variant="caption" fontWeight="bold" sx={{ color: '#666', mb: 0.5, display: 'block' }}>01. ACCOMPANIMENT & COMMUNICATION</Typography>
      <Box sx={{ border: '1px solid #000', p: 1, mb: 2 }}>
        <Grid container alignItems="center">
          <Grid item xs={12} sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
            ...
            <Tick checked={!!patient.companionName} /> <Typography fontSize={10} sx={{color:'black', mr: 2}}> YES</Typography>
            <Tick checked={!patient.companionName} /> <Typography fontSize={10} sx={{color:'black'}}> NO</Typography>
          </Grid>
          <Grid item xs={6} sx={{ display: 'flex' }}>
            <Typography variant="caption" sx={{color:'black'}}>Companion Name: &nbsp;</Typography>
            <Typography variant="caption" fontWeight="bold" sx={{color:'black'}}>{patient.companionName || "—"}</Typography>
          </Grid>
          <Grid item xs={6} sx={{ display: 'flex' }}>
            <Typography variant="caption" sx={{color:'black'}}>Relationship: &nbsp;</Typography>
            <Typography variant="caption" fontWeight="bold" sx={{color:'black'}}>{relation || "—"}</Typography>
          </Grid>
          <Grid item xs={12} sx={{ display: 'flex', alignItems: 'center', mt: 1 }}>
            ...
            <Typography variant="caption" fontWeight="bold" sx={{color:'black'}}>{patient.language || "—"}</Typography>
          </Grid>
        </Grid>
      </Box>

      {/* 03. VITALS & MEASUREMENTS */}
      <Typography variant="caption" fontWeight="bold" sx={{ color: '#666', mb: 0.5, display: 'block' }}>02. VITALS & ANTHROPOMETRY</Typography>
      <Box sx={{ border: '1px solid #000', p: 1, mb: 2 }}>
        <Grid container sx={{ mb: 1 }}>
          <Grid item xs={3} sx={{ fontSize: '11px', color:'black' }}>Temp: <b>{vitals.temp} °C</b></Grid>
          <Grid item xs={3} sx={{ fontSize: '11px', color:'black' }}>HR: <b>{vitals.hr} bpm</b></Grid>
          <Grid item xs={3} sx={{ fontSize: '11px', color:'black' }}>RR: <b>{vitals.rr} bpm</b></Grid>
          <Grid item xs={3} sx={{ fontSize: '11px', color:'black' }}>SpO2: <b>{vitals.spo2} %</b></Grid>
        </Grid>
        <Grid container>
          <Grid item xs={3} sx={{ fontSize: '11px', color:'black' }}>Weight: <b>{vitals.weight} g</b></Grid>
          <Grid item xs={3} sx={{ fontSize: '11px', color:'black' }}>Length: <b>{vitals.length} cm</b></Grid>
          <Grid item xs={3} sx={{ fontSize: '11px', color:'black' }}>HC: <b>{vitals.hc} cm</b></Grid>
          <Grid item xs={3} sx={{ fontSize: '11px', color:'black' }}>BSL: <b>{vitals.bsl} mg/dL</b></Grid>
        </Grid>
        {vitals.relatedText && (
           <Typography variant="caption" sx={{ display: 'block', mt: 1, fontStyle: 'italic' }}>Notes: {vitals.relatedText}</Typography>
        )}
      </Box>

      {/* 04. ALLERGIES */}
      <Typography variant="caption" fontWeight="bold" sx={{ color: '#666', mb: 0.5, display: 'block' }}>03. ALLERGIES / ADVERSE REACTIONS</Typography>
      <Box sx={{ border: '1px solid #000', p: 1, mb: 2 }}>
        <Grid container>
          <Grid item xs={4} sx={{ fontSize: '11px' }}>Medication: <b>{allergies.medication}</b></Grid>
          <Grid item xs={4} sx={{ fontSize: '11px' }}>Blood: <b>{allergies.bloodTransfusion}</b></Grid>
          <Grid item xs={4} sx={{ fontSize: '11px' }}>Food: <b>{allergies.food}</b></Grid>
        </Grid>
      </Box>
    </Stack>
    
    <Typography variant="subtitle2" fontWeight="bold" sx={{px: 2, mb: 1, fontSize: '13px', color:'black' }}>
      04. Current Medications
    </Typography>

    <TableContainer sx={{ px: 2, mb: 4 }}>
      <Table size="small" sx={{ border: '1px solid #000' }}>
        <TableHead sx={{ backgroundColor: '#eee' }}>
          <TableRow>
            <TableCell sx={{ border: '1px solid #000', fontWeight: 'bold', fontSize: '11px', color:'black' }}>Medication</TableCell>
            <TableCell sx={{ border: '1px solid #000', fontWeight: 'bold', fontSize: '11px', color:'black' }}>Dose</TableCell>
            <TableCell sx={{ border: '1px solid #000', fontWeight: 'bold', fontSize: '11px', color:'black' }}>Frequency</TableCell>
            <TableCell sx={{ border: '1px solid #000', fontWeight: 'bold', fontSize: '11px', color:'black' }}>Last Dose Date/Time</TableCell>
          </TableRow>
        </TableHead>
        <TableBody>
          {medications.map((m, i) => (
            <TableRow key={i}>
              <TableCell sx={{ border: '1px solid #000', fontSize: '10px' }}>{m.name || "—"}</TableCell>
              <TableCell sx={{ border: '1px solid #000', fontSize: '10px' }}>{m.dose || "—"}</TableCell>
              <TableCell sx={{ border: '1px solid #000', fontSize: '10px' }}>{m.frequency || "—"}</TableCell>
              <TableCell sx={{ border: '1px solid #000', fontSize: '10px' }}>{m.lastDose ? new Date(m.lastDose).toLocaleString() : "—"}</TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
  ...
  <Card sx={{ ...A4_PAGE_STYLE, p: 2, boxShadow: "none", position: 'relative' }}>
     <Box sx={{  borderBottom: '1px solid grey', p: 1, mb: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      ...
      <Typography variant="subtitle2" fontWeight="bold" sx={{ mb: 1, color: 'black' }}>05. Nursing Needs</Typography>
      <Table size="small" sx={{ border: '1px solid #000', mb: 1 }}>
        <TableBody>
          {[
            ...
            <TableRow key={idx}>
              <TableCell sx={{ py: 0.5, border: '1px solid #000', fontSize: '11px' }}>{item.label}</TableCell>
              <TableCell sx={{ py: 0.5, border: '1px solid #000', textAlign: 'center' }}>
                YES <Tick checked={!!nursingNeeds[item.key as keyof typeof nursingNeeds]} />
              </TableCell>
              <TableCell sx={{ py: 0.5, border: '1px solid #000', textAlign: 'center' }}>
                NO <Tick checked={!nursingNeeds[item.key as keyof typeof nursingNeeds]} />
              </TableCell>
            </TableRow>
          ))}
          <TableRow>
            <TableCell colSpan={3} sx={{ border: '1px solid #000', fontSize: '11px', p: 1 }}>
              <strong>Other Nursing Needs:</strong> {nursingNeeds.otherNeeds || "None"}
            </TableCell>
          </TableRow>
        </TableBody>
      </Table>
