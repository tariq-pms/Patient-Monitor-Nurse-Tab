...
export const NICUDischargeModal: React.FC<NICUDischargeModalProps> = ({
    ...
    const saveToFHIR = async (status: 'Draft' | 'Final') => {
        ...
        try {
            ...
            // Unique ID for this discharge summary
            const summaryId = `discharge-summary-${patient_id}-${new Date().getTime()}`;
            ...
            console.log(`Attempting to save ${status} to:`, `${FHIR_URL}/DocumentReference`);

            const response = await fetch(`${FHIR_URL}/DocumentReference`, {
                method: 'POST',
