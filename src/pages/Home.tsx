...
// })
  ...
  useEffect(() => {
    ...
    const socket = new WebSocket("ws://13.127.51.218:9444/fhir-server/api/v4/notification");
    socket.onopen = () => {
    ...
    socket.onmessage = (data) => {
      ...
      if (recieved_data.location.split("/")[0] == "Observation"){
        // if (obsArray.includes(recieved_data.resourceId)){
          fetch(`http://13.127.51.218:9444/fhir-server/api/v4/${JSON.parse(data.data).location}`, {
          credentials: "omit",
          headers: {
            Authorization: "Basic "+ btoa("fhiruser:change-password"),
            },
          ...
          .then((response) => response.json())
          .then((data) => {
            var temp = String(data.device.reference.split("/")[1])
  ...
  useEffect(() => {
    if(value!="0"){
      setLoading(true)
      fetch(`http://13.127.51.218:9444/fhir-server/api/v4/Device?location=${value}`, {
        credentials: "omit",
        headers: {
          Authorization: "Basic "+ btoa("fhiruser:change-password"),
        },
  ...
  useEffect(() => {
    devices.entry?.map((device) => {
      ...
      if(device.resource.patient){
        fetch(`http://13.127.51.218:9444/fhir-server/api/v4/Observation?patient=${device.resource.patient.reference.split("/")[1]}&_sort=-date&_count=1`, {
        credentials: "omit",
        headers: {
          Authorization: "Basic "+ btoa("fhiruser:change-password"),
          },
        ...
        fetch(`http://13.127.51.218:9444/fhir-server/api/v4/Communication?sender=${device.resource.id}&_count=1`, {
        credentials: "omit",
        headers: {
          Authorization: "Basic "+ btoa("fhiruser:change-password"),
          },
